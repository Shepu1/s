...
function resetIdleTimer() {
    ...
    }, 10000); // Wait 10 seconds after active input/typing completion
    
    questionTimer = setTimeout(askProactiveQuestion, 25000); // Proactive question at 25 seconds
}
...
${retrievedContext}`;
    ...
    try {
        ...
        if ((response.status === 429 || response.status === 403 || response.status === 500 || response.status === 503) && retryCount < 1 && typeof ShepuAPI !== 'undefined') {
            console.warn(`[Shepu Gemini] HTTP Error ${response.status}. Rotating key and retrying...`);
            ShepuAPI.rotateKey();
        ...
        if (data.error && (data.error.code === 429 || data.error.code === 403 || data.error.status === "RESOURCE_EXHAUSTED") && retryCount < 1 && typeof ShepuAPI !== 'undefined') {
            console.warn("[Shepu Gemini] Body error indicates rate limit/quota limit. Rotating key and retrying...");
            ShepuAPI.rotateKey();
            return await callGeminiAPI(query, retryCount + 1);
        }
        ...
        clearTimeout(timeoutId);
        if (retryCount < 1 && typeof ShepuAPI !== 'undefined') {
            console.warn("[Shepu Gemini] Exception caught. Rotating key and retrying...", err.message);
            ShepuAPI.rotateKey();
            return await callGeminiAPI(query, retryCount + 1);
        }
        throw err;
...
async function sendMessage() {
    ...
    if (answer) {
        ...
        setTimeout(() => {
            ...
            isBotAnswering = false; // Reset block before rendering
